import XCTest
@testable import FinderSpaceDemo
...
final class DisplayGeometryTests: XCTestCase {
    func testAppKitToAXCoordinatesForDisplaysAboveAndBelow() {
        ...
        XCTAssertEqual(DemoGeometry.quartzRect(above, primaryTop: 900), CGRect(x: 0, y: -1080, width: 1920, height: 1080))
        XCTAssertEqual(DemoGeometry.quartzRect(below, primaryTop: 900), CGRect(x: 0, y: 900, width: 1920, height: 1080))
    }

    func testLeftDisplayRetainsNegativeCoordinatesAndDockInsets() {
        let visible = CGRect(x: -1860, y: 40, width: 1860, height: 1015)
        XCTAssertEqual(DemoGeometry.quartzRect(visible, primaryTop: 900), CGRect(x: -1860, y: -155, width: 1860, height: 1015))
    }
    ...
    func testMovingBetweenDifferentSizedDisplaysPreservesPointSize() throws {
        ...
        let frame = try XCTUnwrap(DemoGeometry.destinationFrame(
        ...
        XCTAssertTrue(DemoGeometry.contains(frame, in: destination))
    }
    ...
    func testWindowMustFitEntirelyRatherThanJustTouchTargetDisplay() {
        ...
        XCTAssertFalse(DemoGeometry.contains(CGRect(x: 1300, y: 100, width: 800, height: 600), in: visible))
        XCTAssertFalse(DemoGeometry.contains(CGRect(x: 1500, y: 900, width: 800, height: 600), in: visible))
        XCTAssertTrue(DemoGeometry.contains(CGRect(x: 1440, y: 25, width: 800, height: 600), in: visible))
    }

    func testInvalidGeometryCannotProduceAMove() {
        let valid = CGRect(x: 0, y: 25, width: 1440, height: 875)
        XCTAssertNil(DemoGeometry.destinationFrame(window: valid, source: valid, destination: .zero))
        let invalid = CGRect(x: CGFloat.nan, y: 0, width: 800, height: 600)
        XCTAssertNil(DemoGeometry.destinationFrame(window: invalid, source: valid, destination: valid))
        XCTAssertFalse(DemoGeometry.contains(invalid, in: valid))
    }
}
