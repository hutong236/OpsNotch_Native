...
| 桌面与窗口 | 输入 `d` 切换桌面，或将原活动窗口移动到同屏/另一显示器的普通桌面，可选择跟随 |
...
激活要移动的普通应用窗口，呼出 Shelf，输入 `d` 并回车。菜单顶部显示源应用；选择“移动当前窗口到…”或“移动当前窗口并跟随到…”后选择目标显示器/桌面。也可继续按住 `⌥` 或 `⌥⇧` 直接选择目标行。普通选择仍只切换桌面。

需要为 **Ops Notch** 授予辅助功能权限；跨屏独立桌面需要扩展显示并开启“显示器具有单独的空间”。窗口应非全屏、非最小化且只属于一个普通桌面；目标显示器当前也应为普通桌面。窗口尺寸通常保持不变，目标屏较小时会按需缩小到可见区域。操作未完全成功时请检查窗口实际停留的屏幕/桌面，再重新激活窗口重试。

构建和自动检查不代表所有外部 App 均已验证，真机回归步骤见 [VERIFY_ON_MAC.md](VERIFY_ON_MAC.md)。
