...
1. 聚焦唤起:应用正在运行时,聚焦(Spotlight)输入 "Ops Notch" 回车,Shelf 在鼠标所在屏幕展开,搜索框已聚焦可直接打字,前台应用不被切换。
2. 再按一次回车(应用仍在运行):Shelf 收起(切换语义)。
3. 冷启动静默:应用未运行时聚焦回车启动,应用正常初始化但面板不自动展开;再次回车才唤起。
4. 开机静默:登录项开机自启后面板不弹出。
5. 拖放忽略:drop 态时聚焦回车,面板状态不变,拖放正常完成。
6. Finder 双击 / `open` 命令打开已运行的应用:与聚焦回车同语义。
7. App 快捷指令短语(macOS 26 运行时发现,无构建期元数据):聚焦或快捷指令 App 中出现"打开 Ops Notch 清单"动作,执行后面板行为与热键一致;若系统未收录该短语,则按提案回退(移除 SummonIntent),仅保留打开事件入口。
