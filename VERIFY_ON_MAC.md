...
窗口定位回归：在持续隐藏区分别右键/左键 Clash Verge、OneDrive、TCP Latency；保持整个区域收起，检查菜单能打开且选项可用。如仍失败，使用错误提示下的“复制诊断信息”记录 AX 矩形、窗口编号与层级；不要仅把该提示当作权限问题。对子控件带边距、窗口被平移、点击前窗口改变尺寸分别复测。
...
3. 检查 Control-click 等同右键；连续快速点击不重复触发或把刚打开的菜单关闭。菜单外点击和 Esc 能正常退出。
4. 面板已有缓存后移动图标，点击使用当前窗口位置；关闭目标 App 后点击旧条目应提示刷新，不点击其他窗口。
5. 目标读取期间关闭面板、刷新、停用管理、切换隐藏状态或拔出显示器，过期点击不得继续投递。撤销辅助功能权限后显示权限提示。
