# shelf-kind-filter Specification

## Purpose
让 Quick Shelf 在条目较多时可以按类型收窄列表:搜索栏下方提供类型筛选 chips(全部 / 文件 / 文本 / URL / 应用 / 安全操作),与搜索词叠加生效,并与键盘取回流(高亮导航)打通——⌘1~⌘6 切换筛选、筛选变化后高亮回落首行、Space 预览高亮条目。
